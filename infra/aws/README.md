...
**user storage** and the **Vercel dashboard**'s access to that storage — in one
root sharing one state file, plus `bootstrap/`, which is applied by hand and
creates the things that cannot create themselves.

The runbook is `DEPLOYING.md`: the first apply, verification, rollback, and the
Vercel OIDC setup.
...
`backend.tf` keys the state `briefing-worker/terraform.tfstate`, a name left
over from a stack that has since been removed. It stays: changing the key points
this root at an empty state file and orphans every resource in the real one.
...
- **One variable per stack, not one per module input.** The root's whole
  interface is four names: `region`, `alert_email`, `user_storage`,
  `vercel_dashboard`. Stack number three makes it five.
...
```mermaid
flowchart TB
  app["Dashboard on Vercel<br>uploads and downloads"]
  role(["control-panel-vercel-dashboard<br>assumed via Vercel OIDC"])
  ...
  lc["Lifecycle — by tag, not prefix<br>resumes never expire"]
...
| Lifecycle **per kind, by tag** | resumes never expire automatically; superseded versions are kept a year                  |
...
| Dashboard role                 | Vercel OIDC trust pinned to one team, project and environment; holds `prod:resumes`      |
| SNS topic                      | the one alerting destination, with an email subscription                                 |
...
a key you control and rotate — worth revisiting given that the bucket holds
uploaded personal documents. The module then adds `kms:Decrypt` and
`kms:GenerateDataKey` to each access policy and enables an S3 Bucket Key, which
is what keeps per-object KMS calls from dominating the bill.

## One environment

`environments = ["prod"]`. That is the whole story, and it is a statement of fact
rather than an aspiration: there is one deployment, one state file and one
bucket.

A `dev` entry in that list is not a dev environment — it is a key prefix nothing
writes to and IAM policies nothing is attached to. A second environment, when it
exists, is a second state file, not a second list entry.

## Least privilege, per environment and per kind

Splitting by environment means the identity running production cannot name a
`dev/` key. Splitting by kind means a workload that needs one category of user
data holds no grant over any other — including whichever kind is added next.

The dashboard holds exactly `prod:resumes`, and that is asserted rather than
described: `tests/vercel_dashboard.tftest.hcl` fails if the set of attached
policies changes, so a further kind reaching the dashboard has to be argued for
there.
...
```json
{
  ...
  "Resource": "arn:aws:s3:::<bucket>/prod/*/resumes/*"
},
{
  "Sid": "ListOneKindOnly",
  "Effect": "Allow",
  ...
  "Condition": { "StringLike": { "s3:prefix": "prod/*/resumes/*" } }
}
```
...
to it**, exactly as `vercel-dashboard.tf` does. Forgetting is not a silent
...
If a new stack's roles are not named `control-panel-*`, check
`managed_role_arn_patterns` in `bootstrap/variables.tf` and add a pattern if none
matches. That too fails loudly rather than quietly.
...
| `alerts_topic_arn`              | the one topic any stack's alarms publish to             |
...
under `tests/` runs a mocked plan and asserts on what it produces — every object
kind carrying a lifecycle rule, all four public-access-block flags set, the
dashboard attached to exactly the kinds it is meant to hold, an unconfigured
dashboard stack creating nothing. It needs no credentials, no network and no
state, so it runs on a pull request.
...
`modules/user-storage` creates **no IAM role**. A workload's role belongs to the
...
1. Attach `user_storage_kind_policy_arns["<environment>:<kind>"]` to the
   workload's role — the narrow grant. Use `user_storage_policy_arns` only when a
   workload genuinely touches every kind. `vercel-dashboard.tf` is the worked
   example.
...
3. Call `createS3UserObjectStore()` once at the composition root, wrap it in
   `createResumeStore`, and pass that down. See
