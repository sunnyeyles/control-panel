...
gh variable set USER_STORAGE_BUCKET_NAME --body "control-panel-user-storage-<account-id>"
```

`USER_STORAGE_BUCKET_NAME` names the bucket the user-storage stack creates, and
is not a bootstrap output — that stack has no default for it, so pick the name
here. It is separate from the state bucket above, and globally unique like any
S3 bucket.
