...
- [Worker's place in the monorepo and packaging](tickets/06-monorepo-placement-and-packaging.md) — new app `apps/briefing-worker` (package `@workspace/briefing-worker`; the `apps/dashboard`-is-`web` naming mismatch this ticket warned about no longer exists — it's `@workspace/dashboard` now). Trigger-vs-task seam: `functions/scheduledRun.ts` (shallow, just `app.timer()`) calls `runScheduledTask.ts` (deep, owns ticket 03's whole task/success contract). Depends on `agents-core`+`agent-tools` only for now, not `@workspace/agents`. esbuild (invoked directly via `build.mjs`, chosen by the human over tsup) bundles to one dependency-free ESM file, Node 22, no turbo.json changes needed.
- [IaC tool, resource structure, and secrets](tickets/07-iac-resource-structure-and-secrets.md) — **azd + Bicep**, `azure.yaml`/`infra/` at repo root, single resource group `rg-briefing` under azd environment name `briefing` (naming otherwise follows the azd starter's own CAF-abbreviation convention, not a hand-rolled one). **Single environment, no dev/prod split** — resolves the multi-environment fog item. `OPENAI_API_KEY` in Key Vault (`openai-api-key`), Key Vault reference + the managed identity from ticket 05; value entry is a one-time manual `az keyvault secret set`, never in the tracker. Cadence stays in-code per ticket 05; concrete value `0 0 9 * * *` UTC.
- [CI/CD pipeline shape](tickets/08-cicd-pipeline-shape.md) — **GitHub Actions, scaffolded by `azd pipeline config`**; OIDC federated credentials, no stored service-principal secret. Push-to-`main` + manual dispatch, one combined pipeline (`azd up`-equivalent) rather than separate infra/code deploys. Path-scoped `on.push.paths` keeps dashboard-only changes from triggering it at all; the build step still runs through `pnpm turbo build --filter` for correct dependency ordering.
- [Cost guardrails](tickets/10-cost-guardrails.md) — **budget now, in Bicep, not deferred**: subscription-scoped `Microsoft.Consumption/budgets` at USD 5/month, alerts at 50/90/100% actual + 100% forecast, alert email via azd env parameter. Rationale: free to ship, and the guardrail must already exist the day the Free Trial's spending limit disappears. Notify-only (no hard stop after trial); confirm budget support on the trial offer at implementation time.
- [Handoff spec assembled](tickets/09-assemble-handoff-spec.md) — the destination deliverable: [assets/handoff-spec.md](assets/handoff-spec.md), a seven-step build sequence carrying every decided value, four implementation-time conditionals, and ticket 03's "done when" criteria. **All tickets on this map are closed; the map is complete.**

## Not yet specified

_(none — the multi-environment story was resolved by ticket 07: single environment.)_
