...
- [Worker's place in the monorepo and packaging](tickets/06-monorepo-placement-and-packaging.md) — new app `apps/briefing-worker` (package `@workspace/briefing-worker`; the `apps/dashboard`-is-`web` naming mismatch this ticket warned about no longer exists — it's `@workspace/dashboard` now). Trigger-vs-task seam: `functions/scheduledRun.ts` (shallow, just `app.timer()`) calls `runScheduledTask.ts` (deep, owns ticket 03's whole task/success contract). Depends on `agents-core`+`agent-tools` only for now, not `@workspace/agents`. tsup bundles to one dependency-free file, Node 22, no turbo.json changes needed.
