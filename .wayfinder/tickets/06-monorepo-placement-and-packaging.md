...
**Packaging**: per ticket 05 (Flex Consumption, zip deploy) and ticket 02's research (pnpm symlinks don't survive Azure run-from-package), the worker is bundled with **tsup** into a single dependency-free JS file — no `node_modules` in the deploy artifact, so none of the `workspace:*`/symlink problems apply. Config lives at `apps/briefing-worker/tsup.config.ts`, entry `src/index.ts`, target `node22` (see version note below), format `esm` (Functions v4 Node model supports ESM), output to `apps/briefing-worker/dist/index.js`. `@azure/functions` has no native dependencies, so it's bundled in too rather than left external — keeping the artifact to one file plus `host.json`. A `zip` script (`package.json` → `"zip": "cd dist && zip -r ../functionapp.zip . && cd .. && zip -j functionapp.zip host.json"`, or equivalent) produces the deploy artifact ticket 08's pipeline pushes; no Dockerfile exists anywhere in this app, correctly, since Flex Consumption zip-deploy needs none.
