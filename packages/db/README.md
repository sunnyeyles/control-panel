...
The script runs `src/migrate-cli.ts` through `tsx`. No build step is required
first, and `tsconfig.json` excludes the file from emit, so there is no compiled
copy in `dist/` to run stale by mistake — `src` is the only way in.
