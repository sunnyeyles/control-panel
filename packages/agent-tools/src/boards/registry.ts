...
import { createLinkedinSearch, LINKEDIN_TOOL_NAME } from "./linkedin-search.ts"
import type { PostingCatalog } from "./posting-catalog.ts"
import type { SearchLog } from "./search-log.ts"
import { createSeekSearch, SEEK_SPEC, SEEK_TOOL_NAME } from "./seek-search.ts"
 ...
 *
 * ⚠️ **This table lives beside the specs it names, not up in
 * `@workspace/agents`.** Every field on a row except how `postingId` consumes
 * it is a fact about the board, and the spec, the search factory and the by-URL
 * fetcher are all in this directory — so adding a board is one file here plus
 * one row, rather than a matching edit in two packages. What stays up there is
 * the platform's *use* of these facts: `postingId` hashes a URL after dropping
 * the tracking parameters a row declares, and `board-fetch.ts` wires
 * `fetchByUrl` to it. `@workspace/agents/job-boards` re-exports this module so
 * that arrangement is invisible to anything that only wants `boardForHost`.
 *
