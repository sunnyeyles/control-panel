...
**A `SessionEnd` hook deletes worktrees whose work is already published.** `.claude/hooks/cleanup-merged-worktrees.sh`, wired up in `.claude/settings.json`, removes a checkout under `.claude/worktrees/` and its local branch once every commit it holds exists somewhere else — on the default branch, on a remote branch, or as the head of a pull request GitHub still knows about. Merged is the obvious case; an open one counts too, because a review is answered from a fresh `gh pr checkout` and holding the original checkout open buys nothing. **An unpushed commit is the only thing here that cannot be got back, and it is the thing the hook exists to protect** — that, a live `claude` process holding the worktree, and uncommitted changes are what make it refuse. Being unable to reach GitHub is always a reason to keep, never to remove.

That evidence is graded, and the grading is the part worth knowing. A pull request sitting on exactly this tip is **strong** and collects immediately. Merely being contained in `origin/main`, or merely being on some remote ref, is **weak** — true of a brand-new worktree that has not been committed to yet, which sits at the default branch tip and is therefore trivially "published" while being exactly the thing you are about to work in. Weak evidence collects only after `WORKTREE_CLEANUP_MIN_IDLE_HOURS` (72) of inactivity, measured from the mtime of `.git/worktrees/<id>/index`. Every decision, keep or remove, is logged with a reason to `~/.claude/worktree-cleanup.log`, which is trimmed rather than grown forever; nothing is written and no network call is made when there is nothing to consider. `WORKTREE_CLEANUP_DRY_RUN=1` logs what it would do and removes nothing, and `WORKTREE_CLEANUP_GATE=pr` restores the older "only a finished pull request counts" behaviour. **The ladder is tested** — `bash .claude/hooks/cleanup-merged-worktrees.test.sh`, run by hand rather than through `turbo test`, since it builds throwaway repos and stubs `gh` on `PATH`.
