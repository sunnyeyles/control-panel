...
import { OBJECT_GONE, storageMessage } from "@/lib/actions/storage-message"
    ...
    try {
      ...
      return fail(
        storageMessage({
          ...
          operation: "upload",
          error,
          // Reachable here, and not only through a bad extension:
          ...
          // because the extension is fine. The user has a file in front of them
          // to check.
          invalidKey:
    ...
    try {
      ...
      return {
        ...
        message: storageMessage({
          ...
          operation: "delete",
          error,
          // No file and no name to check — the id came from a hidden field the
          // user never saw. `resumeIdSchema` should have caught it first, so
          // reaching here means the row is unaddressable, which is the same
          // thing as gone as far as anyone can act on it.
          invalidKey: OBJECT_GONE,
        }),
      }
    }
